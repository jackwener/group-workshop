...
| 500 | `UPSTREAM_ERROR` | Agent 编排内部异常 | `{"reason":"..."}` |
...
| `records[].answer_source` | string | 是 | 来源：copaw/bailian/demo |
| `records[].response_time_ms` | integer | 是 | 响应耗时（ms） |
| `records[].timestamp` | string | 是 | 记录时间 |
...
| POST /sessions | `title` | 可选，≤ 23 字符 | 400 | `INVALID_TITLE` |
| DELETE /sessions/<id> | `id` | 存在的会话 ID | 404 | `SESSION_NOT_FOUND` |
| GET /sessions/<id>/records | `id` | 存在的会话 ID | 404 | `SESSION_NOT_FOUND` |
