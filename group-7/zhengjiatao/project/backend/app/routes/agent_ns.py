...
create_session_model = agent_ns.model('CreateSessionRequest', {
    'title': fields.String(description='会话标题（可选，默认"新会话"）', example='投资分析', max_length=23)
})
...
record_model = agent_ns.model('Record', {
    'record_id': fields.String(description='记录ID'),
    'session_id': fields.String(description='会话ID'),
    'query': fields.String(description='用户问题'),
    'answer': fields.String(description='AI回答'),
    'llm_used': fields.String(description='使用的LLM'),
...
capabilities_response_model = agent_ns.model('CapabilitiesResponse', {
    'traceId': fields.String(description='链路追踪ID'),
    'capabilities': fields.List(fields.Nested(capability_model), description='能力列表'),
    'llm_status': fields.Raw(description='LLM状态')
})
...
class SessionExport(Resource):
    ...
    @agent_ns.param('format', '导出格式', enum=['json', 'txt'], default='json')
    @agent_ns.response(200, '导出成功')
    @agent_ns.response(400, '参数错误或会话无记录', error_response_model)
    @agent_ns.response(404, '会话不存在', error_response_model)
    def get(self, session_id):
        """
        导出会话记录
        
        将会话记录导出为 JSON 或 TXT 格式文件
        """
        pass
